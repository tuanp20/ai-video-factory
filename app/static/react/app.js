...
function ScrapePanel({ onScrapeResult, addToast, setLoading }) {
  ...
  async function handleScrape() {
    if (!url.trim()) {
      addToast("Vui lòng nhập URL", "error");
      return;
    }
    setScraping(true);
    try {
  ...
  async function handleSheetPreview() {
    if (!sheetUrl.trim()) {
      addToast("Vui lòng dán link Google Sheet", "error");
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        addToast(
          `Tìm thấy ${data.total_links} link — ${data.to_crawl} cần crawl`,
          "success",
        );
      } else {
  ...
  async function handleSheetCrawl() {
    ...
    try {
      ...
      if (data.success) {
        ...
        addToast(
          `Hoàn tất! ${successCount}/${data.total} link crawl thành công. Trạng thái đã được cập nhật trên Sheet.`,
          "success",
        );
        ...
        <div className="form-group">
          ...
          >
            ...
            <div className="dropzone-text">
              Kéo thả file hoặc <strong>click để chọn CSV/Excel</strong>
            </div>
          </div>
        </div>
      )}
        ...
        <div>
          <div className="form-group">
            ...
            <div className="sheet-hint">
              Sheet cần có cột chứa link (URL/Link) và cột trạng thái
              (Status/Trạng thái). Nhớ share Sheet với email Service Account.
            </div>
          </div>
            ...
            <div className="sheet-preview-box">
              <div className="sheet-preview-header">
                <div className="sheet-preview-title">
                  ...
                  <div>
                    ...
                    >
                      {sheetPreview.total_links} link · {sheetPreview.to_crawl}{" "}
                      sẵn sàng · {sheetPreview.to_skip} bỏ qua
                    </span>
                  </div>
                </div>
                <button
                      ...
                      style={{
                        width:
                          sheetProgress.total > 0
                            ? `${(sheetProgress.current / sheetProgress.total) * 100}%`
                            : "0%",
                      }}
                    ></div>
                  </div>
                  <span className="sheet-progress-text">
                    Đang crawl... vui lòng chờ
                  </span>
                </div>
              )}
              ...
              <div className="sheet-stats">
                <div className="sheet-stat">
                  <span
                    className="sheet-stat-value"
                    style={{ color: "var(--accent-primary)" }}
                  >
                    {sheetPreview.total_links}
                  </span>
                ...
                <table className="data-table">
                  ...
                  <tbody>
                    {(sheetPreview.links || []).map((link, i) => (
                      <tr
                        key={i}
                        style={{ opacity: link.should_crawl ? 1 : 0.5 }}
                      >
                        <td
                          style={{
                        ...
                        <td>
                          {link.should_crawl ? (
                            <span className="badge badge-sheet-ready">
                              {link.status || "Sẵn sàng"}
                            </span>
                          ) : (
                            <span className="badge badge-sheet-done">
                              {link.status}
                            </span>
                          )}
                        </td>
          ...
          {sheetResults.length > 0 && (
            <div className="result-box show" style={{ marginTop: "1rem" }}>
              ...
              >
                ✅ Kết quả crawl ({sheetResults.filter((r) => r.success).length}
                /{sheetResults.length})
              </h3>
        ...
        <div className="result-box show">
          ...
          <div className="form-group">
            ...
            <textarea
              ...
              rows="3"
              readOnly
              value={(result.video_prompts || []).join("\n")}
            ></textarea>
          </div>
        </div>
      )}
                ...
                {bulkResults.map((item, i) =>
                  item.error ? (
                    ...
                    <tr key={i}>
                      ...
                      <td>
                        <span className="badge badge-approved">Success</span>
                      </td>
                      <td>
                        <button
                          className="btn btn-primary btn-sm"
...
// =============================================
// Bulk Video Pipeline Panel
// =============================================

function BulkVideoPanel({ prefill, addToast, setLoading }) {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(async () => {
      try {
        const res = await fetch(`/api/bulk-pipeline/${activeJobId}/status`);
        const data = await res.json();
        if (data.success) {
          setJobStatus(data);
  ...
  useEffect(() => {
    if (prefill) {
      setProductName(prefill.title || "");
      setProductDescription(prefill.description || "");
      setProductPrice(prefill.price || "");
      if (prefill.image_url) {
        setProductImageUrl(prefill.image_url);
      }
    }
  }, [prefill]);
  ...
  function handleAudioFiles(e) {
    if (e.target.files.length > 6) {
      addToast("Chỉ cho phép tối đa 6 file âm thanh", "warning");
      return;
    }
  ...
  async function handleStartBulk() {
    ...
    fd.append("use_ai_audio", useAiAudio ? "true" : "false");

    if (!useAiAudio) {
      audioFiles.forEach((af) => fd.append("audio_files", af));
    }

    setSubmitting(true);
    try {
      const res = await fetch("/api/bulk-pipeline/start", {
        method: "POST",
        body: fd,
      });
      const data = await res.json();
      if (data.success) {
        addToast("Đã bắt đầu Bulk Pipeline!", "success");
        setActiveJobId(data.bulk_job_id);
        setJobStatus({ status: "pending", progress_pct: 0 });
      } else {
        addToast(data.detail || "Lỗi khởi tạo Bulk Pipeline", "error");
      }
    } catch (e) {
  ...
  async function handleCancelBulk() {
    if (!activeJobId) return;
    if (!confirm("Bạn có chắc muốn hủy bulk job này?")) return;
    try {
      const res = await fetch(`/api/bulk-pipeline/${activeJobId}/cancel`, {
        method: "POST",
      });
      const data = await res.json();
      if (data.success) {
        addToast("Đã hủy Bulk Job", "info");
        setJobStatus((prev) => ({
          ...prev,
          status: "failed",
    ...
    return (
      ...
      >
        <h4 style={{ marginBottom: "0.5rem" }}>
          Trạng thái Bulk Job #{activeJobId}
        </h4>
        <div
          style={{
            display: "flex",
        ...
        >
          <span className={`badge badge-${jobStatus.status}`}>
            {jobStatus.status}
          </span>
          <span style={{ fontWeight: 600 }}>{jobStatus.progress_pct}%</span>
        ...
        {jobStatus.status === "done" && jobStatus.drive_folder_url && (
          <div
            style={{
              padding: "1rem",
              background: "#e6f9f0",
          ...
          >
            ...
            <a
              href={jobStatus.drive_folder_url}
              target="_blank"
              style={{ color: "#067647", textDecoration: "underline" }}
            >
              Mở thư mục Google Drive chứa 6 video ↗
            </a>
          </div>
        )}
  ...
  return (
    <div className="card" id="bulk-card">
      <div className="card-header">
        <span
          className="phase-badge phase-badge-merge"
          style={{ background: "linear-gradient(135deg, #10b981, #047857)" }}
        >
          Mới
        </span>
        <h3>🏭 Bulk Video Factory (Auto tạo 6 Video)</h3>
      </div>
      ...
      <div className="form-row">
        <div className="form-group">
          ...
          <input
            ...
            placeholder="Thời trang, áo thun, giá rẻ..."
          />
        </div>
        <div className="form-group">
        ...
        <div style={{ display: "flex", gap: "2rem", marginBottom: "0.5rem" }}>
          ...
          >
            <input
              type="radio"
              checked={useAiAudio}
              onChange={() => setUseAiAudio(true)}
            />{" "}
            Dùng AI (Edge-TTS kết hợp nam/nữ)
          </label>
          <label
            style={{
              display: "flex",
          ...
          >
            <input
              type="radio"
              checked={!useAiAudio}
              onChange={() => setUseAiAudio(false)}
            />{" "}
            Upload Audio tự ghi (Tối đa 6 file)
          </label>
        </div>
        ...
        {!useAiAudio && (
          ...
          >
            ...
            {audioFiles.length > 0 ? (
              ...
              >
                ✅ Đã chọn {audioFiles.length} file:{" "}
                {audioFiles.map((f) => f.name).join(", ")}
              </div>
            ) : (
              <div className="text-muted">
                Click hoặc kéo thả để chọn tối đa 6 file âm thanh (.mp3,
                .wav)...
              </div>
            )}
          </div>
      ...
      >
        {submitting ? (
          ...
          "🚀 Auto Tạo 6 Video (Full Pipeline)"
        )}
      </button>
...
function SubmitPanel({ prefill, addToast, setLoading, onJobCreated }) {
  ...
  async function uploadImage(file, setUrl, setPreviewFn, label) {
    ...
    try {
      ...
      if (data.success) {
        setUrl(data.url);
        addToast(
          `Upload ${label} thành công: ${(data.size / 1024).toFixed(0)} KB`,
          "success",
        );
      } else {
  ...
  async function handleSubmit() {
    if (!prompt.trim()) {
      addToast("Vui lòng nhập prompt tạo video", "error");
      return;
    }
    ...
    if (wf && wf.required_inputs) {
      if (wf.required_inputs.includes("image_url") && !imageUrl) {
        addToast("Workflow này yêu cầu ảnh nguồn", "error");
        return;
      }
      if (wf.required_inputs.includes("person_image_url") && !personImageUrl) {
        addToast("Workflow này yêu cầu ảnh nhân vật", "error");
        return;
      }
      if (wf.required_inputs.includes("background_image_url") && !bgImageUrl) {
        addToast("Workflow này yêu cầu ảnh background", "error");
        return;
      }
    }
    ...
    try {
      ...
      if (data.success) {
        addToast(
          `Job #${data.job.id} (${workflowId}) đã được đưa vào hàng đợi! 🚀`,
          "success",
        );
        onJobCreated();
        setPrompt("");
  ...
  const needsPerson =
    selectedWf &&
    selectedWf.required_inputs &&
    selectedWf.required_inputs.includes("person_image_url");
  const needsBg =
    selectedWf &&
    selectedWf.required_inputs &&
    selectedWf.required_inputs.includes("background_image_url");
    ...
    <div className="card" id="phase1-card">
          ...
          <div className="dropzone" style={{ position: "relative" }}>
            ...
            {personPreview ? (
              ...
              <>
                <div className="dropzone-icon">👤</div>
                <div className="dropzone-text">
                  Upload ảnh nhân vật <strong>(sẽ được ghép vào video)</strong>
                </div>
              </>
            )}
          </div>
          ...
          <div className="dropzone" style={{ position: "relative" }}>
            ...
            {bgPreview ? (
              ...
              <>
                ...
                <div className="dropzone-text">
                  Upload ảnh nền <strong>(background cho scene)</strong>
                </div>
              </>
            )}
          </div>
      ...
      <div className="form-row">
        ...
        <div className="form-group">
          ...
          <input
            type="text"
            className="form-input"
            placeholder="Tên video / mô tả ngắn"
            value={title}
      ...
      <div className="form-group">
        ...
        <textarea
          className="form-textarea"
          rows="3"
          placeholder="Mô tả cảnh video cần tạo bằng tiếng Anh..."
          value={prompt}
      ...
      <div className="form-group">
        ...
        <textarea
          className="form-textarea"
          rows="3"
          placeholder="Kịch bản đọc bằng giọng tiếng Việt..."
          value={script}
...
function DriveTabContent({
  ...
  async function handlePreview() {
    if (!driveUrl.trim()) {
      addToast("Vui lòng dán link folder Google Drive", "error");
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        setDriveData(data);
        addToast(
          `Tìm thấy ${data.total_images} ảnh — ${data.pending} chưa xử lý`,
          "success",
        );
      } else {
  ...
  function selectAll() {
    if (!driveData) return;
    const pendingIds = driveData.images
      .filter((img) => img.status !== "done")
      .map((img) => img.drive_file_id);
  ...
  async function handleImport() {
    if (selected.length === 0) {
      addToast("Chọn ít nhất 1 ảnh", "error");
      return;
    }
    setImporting(true);
    try {
      ...
      if (data.success) {
        const successResults = (data.results || []).filter(
          (r) => r.success && r.image_url,
        );
        addToast(
          `Import thành công ${successResults.length}/${data.total} ảnh`,
          "success",
        );
        const finalResults = successResults.map((r) => ({
          url: r.image_url,
          name: r.file_name || "Drive Image",
  ...
  const statusColors = {
    pending: { bg: "#eef4ff", color: "#155eef" },
    done: { bg: "#e6f9f0", color: "#067647" },
    processing: { bg: "#fff8e6", color: "#b54708" },
    failed: { bg: "#fef3f2", color: "#b42318" },
  };
  const pendingCount = driveData
    ? driveData.images.filter((img) => img.status !== "done").length
    : 0;
  ...
  return (
    <div>
      <div className="form-group">
        ...
        <div className="sheet-hint">
          Folder cần được share với email Service Account. Ảnh sau khi import sẽ
          được tự động gán vào các Node trong Workflow.
        </div>
      </div>
        ...
        <div>
          ...
          >
            <div>
              ...
              >
                {driveData.total_images} ảnh · {driveData.pending} chưa xử lý ·{" "}
                {driveData.done} đã xong
              </span>
            </div>
            <div
            ...
            >
              <button
                className="btn btn-outline btn-sm"
                onClick={
                  selected.length === pendingCount ? deselectAll : selectAll
                }
              >
                {selected.length === pendingCount && pendingCount > 0
                  ? "☐ Bỏ chọn tất cả"
              ...
              >
                <select
                  ...
                  value={targetNodeIdx}
                  onChange={(e) => setTargetNodeIdx(e.target.value)}
                >
                  <option value="auto">Gán Auto</option>
                  {Array.from({ length: Math.max(1, nodeCount || 1) }).map(
                    (_, i) => (
                  ...
                  {nodeCount < 10 && (
                    <option value={nodeCount}>Vào Node Mới</option>
                  )}
                </select>
                <button
            ...
            {driveData.images.map((img) => {
              ...
              return (
                <div
                  ...
                  className={`drive-img-card ${isSelected ? "selected" : ""} ${img.status === "done" ? "done" : ""}`}
                  onClick={() =>
                    img.status !== "done" && toggleSelect(img.drive_file_id)
                  }
                >
                  {img.thumbnail_url ? (
                    <img
                      className="drive-img-thumb"
      ...
      {importedImages.length > 0 && (
        <div className="drive-imported-section">
          ...
          >
            ✅ Đã import {importedImages.length} ảnh — Ảnh sẽ được gán tự động
            vào các Node theo thứ tự
          </h4>
            ...
            {importedImages.map((img, i) => (
              <div key={i} className="drive-imported-item">
                <img
                  src={img.url}
                  alt={img.name}
                  className="drive-imported-thumb"
                />
                <div className="drive-imported-label">
                  <span className="drive-imported-node">
                    Node{" "}
                    {(img.targetNodeIdx === "auto"
                      ? i
                      : parseInt(img.targetNodeIdx) + i) + 1}
                  </span>
          ...
          <div className="sheet-hint" style={{ marginTop: "0.5rem" }}>
            💡 Chuyển sang tab "🔨 Tạo Workflow" hoặc "▶️ Chạy Workflow" để xem
            ảnh đã được gán vào các Node.
          </div>
        </div>
      )}
    </div>
...
function WorkflowBuilderPage({ addToast, setLoading }) {
  ...
  function _removeId(id) {
    _saveIds(_loadIds().filter((x) => x !== id));
  }
  ...
  useEffect(() => {
    const iv = setInterval(() => {
      const ids = _loadIds();
      if (ids.length > 0) pollActiveJobs(ids);
    }, 5000);
    return () => clearInterval(iv);
  }, [pollActiveJobs]);
  ...
  function handleDriveImported(images) {
    ...
    addToast(
      `✅ ${images.length} ảnh từ Drive đã được gán vào các Node`,
      "success",
    );
  }
  ...
  function addNode() {
    if (nodes.length >= 10) {
      addToast("Tối đa 10 nodes", "error");
      return;
    }
  ...
  // bulk settings on overall run
  const [bulkProductName, setBulkProductName] = useState("");
  const [bulkAudioUseAi, setBulkAudioUseAi] = useState(true);
  const [bulkAudioFiles, setBulkAudioFiles] = useState([]);

  async function runWorkflow() {
    if (!nodes[0].prompt.trim() && !nodes[0].is_bulk) {
      addToast("Node 1 cần có Prompt", "error");
      return;
    }
    ...
    try {
      if (lastNode.is_bulk) {
        // Run Bulk Workflow Pipeline
        const fd = new FormData();
        fd.append("product_name", bulkProductName || "Bulk Workflow Video");
        fd.append("workflow_nodes_str", JSON.stringify(nodes));
        fd.append("use_ai_audio", bulkAudioUseAi ? "true" : "false");
        if (!bulkAudioUseAi) {
          bulkAudioFiles.forEach((af) => fd.append("audio_files", af));
        }

        const res = await fetch("/api/bulk-pipeline/start-workflow", {
          method: "POST",
          body: fd,
        });
        const data = await res.json();
        if (data.success) {
          addToast("🚀 Đã khởi tạo Bulk Workflow!", "success");
          // You may want to redirect to Dashboard or track it somehow
          // Tracking it inside active Jobs won't work immediately without mapping BulkJob model.
          // But user can see it in Dashboard bulk list.
        } else {
          addToast(data.detail || "Lỗi bulk", "error");
        }
      } else {
        // Standard single workflow
        const res = await fetch("/api/workflow-builder/run", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            title: `Workflow Builder (${nodes.length} nodes)`,
            nodes,
          }),
        });
        const data = await res.json();
        if (data.success) {
          ...
          addToast(
            `Job #${data.job.id} đang xử lý (${nodes.length} nodes) 🚀`,
            "success",
          );
          _addId(data.job.id);
  ...
  async function saveWorkflow() {
    if (!saveName.trim()) {
      addToast("Nhập tên workflow", "error");
      return;
    }
    setSaving(true);
    try {
  ...
  async function deleteWorkflow(wfId) {
    try {
      const res = await fetch(`/api/custom-workflows/${wfId}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (data.success) {
  ...
  async function handleRunnerUpload(file, idx) {
    ...
    try {
      ...
      if (data.success) {
        setRunnerImages((prev) =>
          prev.map((img, i) => (i === idx ? data.url : img)),
        );
        addToast(`Ảnh Node ${idx + 1} uploaded`, "success");
      }
    } catch (e) {
  ...
  async function runSavedWf() {
    ...
    try {
      const res = await fetch(`/api/custom-workflows/${selectedWf.id}/run`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title: runnerTitle || selectedWf.name,
          node_images: runnerImages,
        }),
      });
      const data = await res.json();
      if (data.success) {
        addToast(
          `Job #${data.job.id} đang xử lý workflow "${selectedWf.name}" 🚀`,
          "success",
        );
      ...
      <React.Fragment key={idx}>
        {idx > 0 && (
          <div className="wf-node-connector">
            <span className="arrow-down">⬇</span> Output Node {idx} → Input Node{" "}
            {idx + 1}
          </div>
        )}
        <div className="wf-node">
          ...
          <div className="form-group">
            ...
            {editable ? (
              <div style={{ display: "flex", gap: "0.5rem" }}>
                <button
                  className={`btn btn-sm ${!isImage ? "btn-primary" : "btn-outline"}`}
                  onClick={() => {
                    updateNode(idx, "category", "video");
                    updateNode(idx, "model", "kling-3.0");
                    updateNode(idx, "aspect_ratio", "9:16");
                  }}
                >
                  🎬 Tạo Video
                </button>
                <button
                  className={`btn btn-sm ${isImage ? "btn-primary" : "btn-outline"}`}
                  onClick={() => {
                    updateNode(idx, "category", "image");
                    updateNode(idx, "model", "nano-banana-pro");
                    updateNode(idx, "aspect_ratio", "1:1");
                  }}
                >
                  🖼️ Tạo Ảnh
                </button>
              ...
              <div>
                <strong>{isImage ? "🖼️ Tạo Ảnh" : "🎬 Tạo Video"}</strong>
              </div>
            )}
          </div>
              ...
              <div>
                ...
                >
                  <input
                    ...
                    accept="image/*"
                    onChange={(e) =>
                      handleUploadForNode(e.target.files[0], idx)
                    }
                    style={{
                      position: "absolute",
                      inset: 0,
                  ...
                  {node.image_url ? (
                    <div className="dropzone-preview">
                      ...
                      >
                        ✅{" "}
                        {node.image_url.includes("r2.") ||
                        node.image_url.includes("cloudflare")
                          ? "Từ Drive"
                          : "Đã upload"}
                      </div>
                    </div>
                    ...
                    <div className="dropzone-text">
                      Click để upload ảnh hoặc import từ tab Google Drive
                    </div>
                  )}
                </div>
                {node.image_url && (
                  <button
                    ...
                    onClick={() => updateNode(idx, "image_url", "")}
                  >
                    ✕ Xóa ảnh
                  </button>
                )}
            ...
            <div className="form-group">
              ...
              {editable ? (
                <select
                  ...
                  value={node.model}
                  onChange={(e) => {
                    updateNode(idx, "model", e.target.value);
                    if (e.target.value === "veo-3-fast")
                      updateNode(idx, "mode", "t2v");
                    else if (!isImage) updateNode(idx, "mode", "i2v");
                  }}
                >
                  {!isImage ? (
                    <>
              ...
              <div className="form-group">
                ...
                {editable ? (
                  <select
                    className="form-select"
                    value={node.mode}
                    onChange={(e) => updateNode(idx, "mode", e.target.value)}
                  >
                    {modeOpts.map(([v, l]) => (
              ...
              <div className="form-group">
                ...
                {editable ? (
                  <select
                    className="form-select"
                    value={node.quality}
                    onChange={(e) => updateNode(idx, "quality", e.target.value)}
                  >
                    <option value="1080p">1080p</option>
                    <option value="720p">720p</option>
                  </select>
              ...
              <div className="form-group">
                ...
                {editable ? (
                  <input
                    ...
                    value={node.duration}
                    onChange={(e) =>
                      updateNode(idx, "duration", parseInt(e.target.value) || 5)
                    }
                    min="3"
                    max="15"
                  />
              ...
              <div className="form-group">
                ...
                {editable ? (
                  <select
                    ...
                    onChange={(e) =>
                      updateNode(idx, "resolution", e.target.value)
                    }
                  >
          ...
          <div className="form-group">
            ...
            {editable ? (
              <select
                className="form-select"
                value={node.aspect_ratio}
                onChange={(e) =>
                  updateNode(idx, "aspect_ratio", e.target.value)
                }
              >
          ...
          <div className="form-group">
            <label>
              Prompt tạo {isImage ? "Ảnh" : "Video"} {isFirst ? "*" : ""}
            </label>
            {editable ? (
              <>
                {idx === nodes.length - 1 && !isImage && (
                  <div
                    style={{
                      marginBottom: "0.5rem",
                      display: "flex",
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={node.is_bulk}
                      onChange={(e) =>
                        updateNode(idx, "is_bulk", e.target.checked)
                      }
                      id={`bulk_toggle_${idx}`}
                    />
                    <label
                      htmlFor={`bulk_toggle_${idx}`}
                      style={{
                        margin: 0,
                        color: "var(--accent-primary)",
                        fontWeight: "600",
                    ...
                    >
                      Bật Bulk Mode: Tạo N Video từ dánh sách{" "}
                      {node.is_bulk ? "(↓ Nhập bên dưới)" : ""}
                    </label>
                  </div>
                )}
                ...
                {node.is_bulk ? (
                  <textarea
                    className="form-textarea"
                    rows="4"
                    placeholder="Nhập nhiều prompts, mỗi dòng 1 prompt để sinh ra nhiều video tương ứng..."
                    value={node.bulk_prompts}
                    onChange={(e) =>
                      updateNode(idx, "bulk_prompts", e.target.value)
                    }
                  ></textarea>
                ) : (
                  <textarea
                    className="form-textarea"
                    rows="2"
                    placeholder={`Mô tả ${isImage ? "ảnh" : "cảnh video"}...`}
                    value={node.prompt}
              ...
              >
                {node.is_bulk ? (
                  <pre>{node.bulk_prompts}</pre>
                ) : (
                  node.prompt || <span className="text-muted">—</span>
                )}
              </div>
            )}
          </div>
            ...
            <div className="form-group">
              ...
              {editable ? (
                <textarea
                  ...
                  value={node.negative_prompt}
                  onChange={(e) =>
                    updateNode(idx, "negative_prompt", e.target.value)
                  }
                ></textarea>
              ) : (
                <div
          ...
          <div className="form-group">
            <label>
              Kịch bản TTS {isImage && "(Chỉ dùng cho bước merge video)"}
            </label>
            {editable ? (
              <textarea
                className="form-textarea"
                rows="2"
                placeholder="Kịch bản đọc (tuỳ chọn)..."
                value={node.script_text}
                onChange={(e) => updateNode(idx, "script_text", e.target.value)}
              ></textarea>
            ) : (
              <div style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>
                {node.script_text || "(không có)"}
              </div>
            )}
          </div>
        </div>
    ...
    >
      <div className="page-header">
        ...
        <p className="subtitle">
          Tạo workflow multi-node hoặc chạy workflow đã lưu
        </p>
      </div>
      ...
      <div className="tab-headers" style={{ marginBottom: "1.5rem" }}>
        ...
        >
          ...
          {driveImages.length > 0 && (
            <span className="tab-badge">{driveImages.length} ảnh</span>
          )}
        </button>
        <button
        ...
        >
          ▶️ Chạy Workflow đã lưu ({savedWfs.length})
        </button>
      </div>
      ...
      {activeJobs.length > 0 && (
        ...
        >
          ...
          {activeJobs.map((job) => {
            ...
            const completedN = nodeStatuses.filter(
              (n) => n.status === "completed",
            ).length;
            const progressPct =
              totalN > 0 ? Math.round((completedN / totalN) * 100) : 0;
            const isFinalizing =
              job.status === "processing" && completedN === totalN;
      ...
      <div
        className="card"
        style={{ display: activeTab === "drive" ? "block" : "none" }}
      >
        <div className="card-header">
          <span
            className="phase-badge"
        ...
        {driveImages.length > 0 && (
          ...
          >
            ...
            >
              ...
              >
                ✅ <strong>{driveImages.length}</strong> ảnh đã sẵn sàng cho
                Workflow
              </span>
              <div style={{ display: "flex", gap: "0.5rem" }}>
                <button
                ...
                <button
                  className="btn btn-success btn-sm"
                  onClick={() => setActiveTab("runner")}
                  disabled={savedWfs.length === 0}
                >
                  ▶️ Chạy Workflow có sẵn
                </button>
              </div>
            </div>
          </div>
      ...
      <div
        className="card"
        style={{ display: activeTab === "builder" ? "block" : "none" }}
      >
        <div className="card-header">
        ...
        <button className="wf-add-node-btn" onClick={addNode}>
          ➕ Thêm Node
        </button>

        {nodes[nodes.length - 1].is_bulk && (
          <div
            className="card"
            style={{
              marginTop: "1.5rem",
          ...
          >
            ...
            >
              ⚙️ Cấu hình Hậu kỳ Bulk Workflow
            </h4>

            <div className="form-group">
              <label>Tên Thư mục Drive (Product Name)</label>
              <input
                type="text"
                className="form-input"
                value={bulkProductName}
                onChange={(e) => setBulkProductName(e.target.value)}
                placeholder="Tên sản phẩm / Tên dự án"
              />
            </div>

            <div className="form-group">
              <label>Cài đặt Audio cho các video</label>
              <div
              ...
              >
                ...
                >
                  <input
                    type="radio"
                    checked={bulkAudioUseAi}
                    onChange={() => setBulkAudioUseAi(true)}
                  />{" "}
                  Dùng AI Audio
                </label>
                <label
                ...
                >
                  <input
                    type="radio"
                    checked={!bulkAudioUseAi}
                    onChange={() => setBulkAudioUseAi(false)}
                  />{" "}
                  Upload Audio tự ghi
                </label>
              </div>
              {!bulkAudioUseAi && (
                ...
                >
                  <input
                    ...
                    accept="audio/*"
                    onChange={(e) =>
                      setBulkAudioFiles(Array.from(e.target.files))
                    }
                    style={{
                      position: "absolute",
                      inset: 0,
                  ...
                  {bulkAudioFiles.length > 0 ? (
                    ...
                    >
                      ✅ Đã chọn {bulkAudioFiles.length} file audio
                    </div>
                  ) : (
                    <div className="text-muted">
                      Click hoặc kéo thả để chọn audio files...
                    </div>
                  )}
                </div>
              )}
        ...
        {jobResult && (
          <div className="result-box show" style={{ marginTop: "1rem" }}>
            <h3
              style={{ color: "var(--accent-secondary)", fontSize: "0.95rem" }}
            >
              ✅ Job #{jobResult.id} đã được tạo
            </h3>
            <p style={{ color: "var(--text-muted)", fontSize: "0.85rem" }}>
              Trạng thái:{" "}
              <span className={`badge badge-${jobResult.status}`}>
                {jobResult.status}
              </span>{" "}
              — Kiểm tra trong trang Dashboard.
            </p>
          </div>
        )}
      </div>
          ...
          {savedWfs.length === 0 ? (
            ...
            <div className="wf-selector-grid">
              {savedWfs.map((wf) => (
                <div
                  key={wf.id}
                  className={`wf-selector-card ${selectedWf && selectedWf.id === wf.id ? "selected" : ""}`}
                  onClick={() => selectSavedWf(wf)}
                >
                  <h4>⚡ {wf.name}</h4>
                  {wf.description && (
          ...
          <div className="card">
            ...
            <div className="form-group">
              ...
              <input
                type="text"
                className="form-input"
                placeholder={selectedWf.name}
                value={runnerTitle}
                onChange={(e) => setRunnerTitle(e.target.value)}
              />
            </div>
            ...
            {selectedWf.nodes.map((node, idx) => (
              <div key={idx} className="wf-runner-node">
                ...
                <div className="wf-runner-node-config">
                  <code>{node.model}</code> · <code>{node.mode}</code> ·{" "}
                  <code>{node.quality}</code> · <code>{node.duration}s</code> ·{" "}
                  <code>{node.aspect_ratio}</code>
                </div>
                {node.prompt && (
                  <div
                  ...
                  >
                    Prompt: {(node.prompt || "").substring(0, 100)}
                    {(node.prompt || "").length > 100 ? "..." : ""}
                  </div>
                )}

                {idx > 0 && (
                  <div
                    className="wf-auto-input"
                    style={{ marginBottom: "0.5rem" }}
                  >
                    📥 Auto: Output từ Node {idx}
                  </div>
                )}
                ...
                <div className="form-group" style={{ marginBottom: 0 }}>
                  ...
                  >
                    <input
                      ...
                      accept="image/*"
                      onChange={(e) =>
                        handleRunnerUpload(e.target.files[0], idx)
                      }
                      style={{
                        position: "absolute",
                        inset: 0,
                    ...
                    {runnerImages[idx] ? (
                      <div className="dropzone-preview">
                        ...
                        >
                          ✅{" "}
                          {driveImages[idx] &&
                          runnerImages[idx] === driveImages[idx].url
                            ? "Từ Drive"
                            : "Đã upload"}
                        </div>
                      </div>
                      ...
                      >
                        Click upload ảnh hoặc import từ tab Drive
                      </div>
                    )}
                  </div>
                  {runnerImages[idx] && (
      ...
      {/* Save Modal */}
      <div
        className={`save-modal-overlay ${showSave ? "show" : ""}`}
        ...
        <div className="save-modal" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="form-group">
            ...
            <input
              type="text"
              className="form-input"
              placeholder="VD: Cinematic Product Showcase"
              value={saveName}
          ...
          >
            Workflow sẽ lưu cấu hình {nodes.length} node(s). Ảnh sẽ không được
            lưu — bạn sẽ upload ảnh mới khi chạy lại.
          </div>
          <div
            style={{
              display: "flex",
                ...
                <React.Fragment key={job.id}>
                  <tr>
                    ...
                    <td>
                      ...
                      {job.raw_video_url && !job.final_video_url && (
                        <a
...
function MergePanel({ addToast }) {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const data = await res.json();
        if (data.success) {
          const withVideo = data.jobs.filter(
            (j) => j.raw_video_url || j.final_video_url,
          );
      ...
      try {
        ...
        if (data.success) {
          ...
          if (data.merge.status === "done") {
            ...
            setMerging(false);
            addToast(
              data.merge.error_message || "Ghép video thất bại",
              "error",
            );
            clearInterval(poll);
          }
  ...
  async function handleMerge() {
    if (selectedIds.length === 0) {
      addToast("Chọn ít nhất 1 video", "error");
      return;
    }
    setMerging(true);
    setMergeResult(null);
  ...
  const selectedJobs = selectedIds
    .map((id) => completedJobs.find((j) => j.id === id))
    .filter(Boolean);
    ...
    <div className="card" id="merge-card">
        ...
        <div className="merge-video-grid">
          {completedJobs.length === 0 ? (
            <div className="merge-empty">
              Chưa có video nào hoàn thành. Tạo video ở Phase 1-2 trước.
            </div>
          ) : (
            completedJobs.map((job) => (
              ...
              >
                ...
                <div className="merge-video-info">
                  <div className="merge-video-title">
                    #{job.id} {(job.title || "").slice(0, 30)}
                  </div>
                  <div className="merge-video-meta">
                    {job.model} · {formatTime(job.created_at)}
                  </div>
                </div>
              </div>
            ))
          ...
          <div className="merge-order-list">
            {selectedJobs.map((job, idx) => (
              ...
              >
                ...
                <span className="merge-order-title">
                  #{job.id} — {(job.title || "Video").slice(0, 40)}
                </span>
                <button
                  className="merge-order-remove"
                  onClick={(e) => {
                    e.stopPropagation();
                    toggleSelect(job.id);
                  }}
      ...
      <div className="form-group">
        ...
        <div className="form-hint">
          Âm thanh từ TikTok sẽ thay thế hoàn toàn audio gốc của video
        </div>
      </div>
      ...
      {mergeStatus && mergeStatus !== "done" && mergeStatus !== "failed" && (
        <div className="merge-status-bar">
          ...
          <div className="sheet-progress-text">
            {statusLabels[mergeStatus] || mergeStatus}
          </div>
        </div>
      )}
      ...
      {mergeResult && mergeResult.merged_video_url && (
        <div className="merge-result">
          <div className="merge-result-header">
            ...
            <a
              href={mergeResult.merged_video_url}
              target="_blank"
              download
              className="btn btn-success btn-sm"
            >
      ...
      {mergeStatus === "failed" && (
        <div className="merge-error">
          ...
          <button className="btn btn-outline btn-sm" onClick={resetMerge}>
            Thử lại
          </button>
        </div>
      )}
  ...
  return (
    ...
    >
      <div className="page-header">
        ...
        <p className="subtitle">
          Tạo video AI hàng loạt — Từ URL bài viết đến video hoàn chỉnh
        </p>
      </div>
...
function ReviewPage({ addToast }) {
  ...
  const loadData = useCallback(async () => {
    try {
      const [pendingRes, approvedRes, rejectedRes, processingRes] =
        await Promise.all([
  ...
  return (
    ...
    >
      <div className="page-header">
        ...
        <p className="subtitle">
          Xem và phê duyệt các video đang chờ kiểm duyệt
        </p>
      </div>
      ...
      {/* Reject Modal */}
      <div
        className={`modal-overlay ${rejectModal ? "show" : ""}`}
...
function App() {
  ...
  function renderPage() {
    switch (page) {
      case "review":
        return <ReviewPage addToast={addToast} />;
      case "workflow-builder":
        return (
          <WorkflowBuilderPage addToast={addToast} setLoading={setLoading} />
        );
      default:
        return <DashboardPage addToast={addToast} setLoading={setLoading} />;
    }
  }
